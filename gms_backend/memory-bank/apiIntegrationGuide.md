# GMS Frontend Development: API Integration Guide
...
1. **Endpoint**: POST /api/v1/auth/login-username
2. **Request Payload**:
   ```json
   {
     "username": "string",
   ...
   ```
3. **Response**: Same as email login
4. **Implementation**:
   - Alternative login method for users who prefer username
   - Same validation and token handling as email login
